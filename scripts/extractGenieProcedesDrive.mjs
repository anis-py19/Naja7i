...
const folderUrl = 'https://drive.google.com/drive/folders/10vgncBODa6BTPFxs3MRNBefm-mow7CL5?usp=sharing';
...
async function extractGenieProcedesDrive() {
  console.log('Fetching Génie des Procédés (هندسة الطرائق) Google Drive folder...');
  const html = await fetchHtml(folderUrl);
  fs.writeFileSync('scripts/drive_procedes_raw.html', html);
  ...
  console.log('Total unique Génie des Procédés Drive files:', unique.length);
  fs.writeFileSync('scripts/procedes_drive_files.json', JSON.stringify(unique, null, 2));
...
extractGenieProcedesDrive().catch(console.error);
