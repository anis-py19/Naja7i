...
const driveFiles = JSON.parse(fs.readFileSync('scripts/noureddine_drive_files.json', 'utf8'));
...
const scientificStreams = ['sciences', 'math', 'technique_math', 'gestion'];

// 1. Update existing matches
files.forEach(f => {
  if (f.subjectId === 'math' || (f.fileUrl && f.fileUrl.includes('/Math/'))) {
...
driveFiles.forEach((df, idx) => {
  if (!df._matched) {
    ...
    const newId = `ffm_noureddine_${Date.now()}_${idx + 1}`;
    ...
    files.push({
      ...
      extension: 'pdf',
      subjectId: 'math',
      subjectAliases: ['math', 'mathematics', 'maths', 'riyadiyat', 'noureddine'],
...
console.log('Updated existing Noureddine files:', updatedCount);
console.log('Appended new Noureddine Drive files:', newCount);
