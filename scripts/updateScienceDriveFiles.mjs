...
const driveFiles = JSON.parse(fs.readFileSync('scripts/science_drive_files.json', 'utf8'));
...
files.forEach(f => {
  if (f.subjectId === 'sciences_nat' || f.subjectId === 'science' || f.subjectId === 'biology' || (f.fileUrl && (f.fileUrl.includes('/Science/') || f.fileUrl.includes('/Sciences/')))) {
...
driveFiles.forEach((df, idx) => {
  if (!df._matched) {
    ...
    const newId = `ffm_sci_${Date.now()}_${idx + 1}`;
    
    let category = 'ملخصات ودروس';
    if (cleanTitle.includes('نصوص علمية') || cleanTitle.includes('النصوص العلمية')) category = 'مقالات ونصوص علمية';
    else if (cleanTitle.includes('تمارين') || cleanTitle.includes('سلسلة')) category = 'سلاسل وتمارين';
    else if (cleanTitle.includes('كتاب') || cleanTitle.includes('السلسلة الخضراء')) category = 'كتب ومذكرات شاملة';

    const author = cleanTitle.includes('بن خريف') ? 'الأستاذ مصطفى بن خريف'
      : cleanTitle.includes('فراح عيسى') ? 'الأستاذ فراح عيسى'
      : 'أساتذة متميزون';
