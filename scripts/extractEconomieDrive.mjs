...
const folderUrl = 'https://drive.google.com/drive/folders/1hCZk8KK1OL940HDqyDfrKiaW2-9SWDXH?usp=sharing';
...
async function extractEconomieDrive() {
  console.log('Fetching Economie (الاقتصاد والمناجمنت) Google Drive folder...');
  const html = await fetchHtml(folderUrl);
  fs.writeFileSync('scripts/drive_economie_raw.html', html);
  ...
  console.log('Total unique Economie Drive files:', unique.length);
  fs.writeFileSync('scripts/economie_drive_files.json', JSON.stringify(unique, null, 2));
...
extractEconomieDrive().catch(console.error);
