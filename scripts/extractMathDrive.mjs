...
const folderUrl = 'https://drive.google.com/drive/folders/13zmessBg9iJe8suQ_O3pizRVOIG271MF?usp=sharing';
...
async function extractMathDrive() {
  console.log('Fetching Math (الرياضيات) Google Drive folder...');
  const html = await fetchHtml(folderUrl);
  fs.writeFileSync('scripts/drive_math_raw.html', html);
  ...
  console.log('Total unique Math Drive files:', unique.length);
  fs.writeFileSync('scripts/math_drive_files.json', JSON.stringify(unique, null, 2));
...
extractMathDrive().catch(console.error);
