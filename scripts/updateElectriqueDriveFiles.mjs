...
const driveFiles = JSON.parse(fs.readFileSync('scripts/electrique_drive_files.json', 'utf8'));
...
files.forEach(f => {
  if (f.subjectId === 'genie_electrique' || (f.fileUrl && (f.fileUrl.includes('/GenieElectrique/') || f.fileUrl.includes('/Electrique/')))) {
...
driveFiles.forEach((df, idx) => {
  if (!df._matched) {
    ...
    const newId = `ffm_elec_${Date.now()}_${idx + 1}`;
    
    let category = 'ملخصات ودروس';
    if (cleanTitle.includes('حوليات') || cleanTitle.includes('تمارين')) category = 'حوليات وسلاسل تمارين';

    const author = cleanTitle.includes('بن التاج') ? 'الأستاذة بن التاج فتيحة' : 'أساتذة متميزون';
    ...
    files.push({
      ...
      subjectAliases: ['genie_electrique', 'electrique', 'ge'],
...
console.log('Updated existing Génie Électrique files:', updatedCount);
console.log('Appended new Génie Électrique Drive files:', newCount);
