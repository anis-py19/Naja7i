...
const folderUrl = 'https://drive.google.com/drive/folders/1fwgqL0C25gDXAMC6YK6kD35LjKGuj2p7?usp=sharing';
...
async function extractIslamiyaDrive() {
  console.log('Fetching Islamiya (العلوم الإسلامية) Google Drive folder...');
  const html = await fetchHtml(folderUrl);
  fs.writeFileSync('scripts/drive_islamiya_raw.html', html);
  ...
  console.log('Total unique Islamiya Drive files:', unique.length);
  fs.writeFileSync('scripts/islamiya_drive_files.json', JSON.stringify(unique, null, 2));
...
extractIslamiyaDrive().catch(console.error);
