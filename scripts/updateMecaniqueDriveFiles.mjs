...
const driveFiles = JSON.parse(fs.readFileSync('scripts/mecanique_drive_files.json', 'utf8'));
...
files.forEach(f => {
  if (f.subjectId === 'genie_mecanique' || (f.fileUrl && (f.fileUrl.includes('/GenieMecanique/') || f.fileUrl.includes('/Mecanique/')))) {
...
driveFiles.forEach((df, idx) => {
  if (!df._matched) {
    ...
    const newId = `ffm_mec_${Date.now()}_${idx + 1}`;
    ...
    files.push({
      ...
      subjectAliases: ['genie_mecanique', 'mecanique', 'gm', 'mecanic'],
...
console.log('Updated existing Génie Mécanique files:', updatedCount);
console.log('Appended new Génie Mécanique Drive files:', newCount);
