...
const folderUrl = 'https://drive.google.com/drive/folders/16dDbPJekXNNM1fzQX9aShadlzLgGrkf7?usp=sharing';
...
async function extractPhiloDrive() {
  console.log('Fetching Philo (الفلسفة) Google Drive folder...');
  const html = await fetchHtml(folderUrl);
  fs.writeFileSync('scripts/drive_philo_raw.html', html);
  ...
  console.log('Total unique Philo Drive files:', unique.length);
  fs.writeFileSync('scripts/philo_drive_files.json', JSON.stringify(unique, null, 2));
...
extractPhiloDrive().catch(console.error);
