...
const driveFiles = JSON.parse(fs.readFileSync('scripts/histgeo_drive_files.json', 'utf8'));
...
files.forEach(f => {
  if (f.subjectId === 'histoire_geo' || f.subjectId === 'histoire' || f.subjectId === 'geographie' || (f.fileUrl && (f.fileUrl.includes('/HistoireGeo/') || f.fileUrl.includes('/Histoire/') || f.fileUrl.includes('/Geo/')))) {
...
driveFiles.forEach((df, idx) => {
  if (!df._matched) {
    ...
    const newId = `ffm_histgeo_${Date.now()}_${idx + 1}`;
    
    let category = 'ملخصات ودروس';
    if (cleanTitle.includes('خرائط') || cleanTitle.includes('خريطة')) category = 'خرائط وتعيين';
    else if (cleanTitle.includes('شخصيات') || cleanTitle.includes('مصطلحات') || cleanTitle.includes('تواريخ')) category = 'مصطلحات وتواريخ وشخصيات';
    else if (cleanTitle.includes('مقالات') || cleanTitle.includes('مقالة')) category = 'منهجية ومقالات جاهزة';
    ...
    else if (cleanTitle.includes('كتاب') || cleanTitle.includes('السلسلة الأرجوانية') || cleanTitle.includes('السلسلة الارجوانية')) category = 'كتب ومذكرات شاملة';

    const author = cleanTitle.includes('بورنان') ? 'الأستاذ بورنان عمار'
      : cleanTitle.includes('محمودي') || cleanTitle.includes('عادل') ? 'الأستاذ محمودي عادل'
      : cleanTitle.includes('قنشوبة') ? 'الأستاذ قنشوبة'
      : cleanTitle.includes('عبدوش') ? 'الأستاذ عبدوش مختار'
      : cleanTitle.includes('حفوظة') ? 'الأستاذ الطاهر حفوظة'
      : 'أساتذة متميزون';

    const streams = cleanTitle.includes('شعب أدبية') || cleanTitle.includes('شعبة ادب')
    ...
    files.push({
      ...
      subjectAliases: ['histoire_geo', 'histoire', 'geographie', 'ijtima3iyat', 'tarikh_joghrapiya'],
...
console.log('Updated existing Histoire-Géo files:', updatedCount);
console.log('Appended new Histoire-Géo Drive files:', newCount);
