...
const folderUrl = 'https://drive.google.com/drive/folders/18ReMaxguSKELPcmHZ1dFmU8ptivTskXd?usp=sharing';
...
async function extractGenieCivilDrive() {
  console.log('Fetching Génie Civil (الهندسة المدنية) Google Drive folder...');
  const html = await fetchHtml(folderUrl);
  fs.writeFileSync('scripts/drive_civil_raw.html', html);
  ...
  console.log('Total unique Génie Civil Drive files:', unique.length);
  fs.writeFileSync('scripts/civil_drive_files.json', JSON.stringify(unique, null, 2));
...
extractGenieCivilDrive().catch(console.error);
