...
const driveFiles = JSON.parse(fs.readFileSync('scripts/procedes_drive_files.json', 'utf8'));
...
files.forEach(f => {
  if (f.subjectId === 'genie_procedes' || (f.fileUrl && (f.fileUrl.includes('/GenieProcedes/') || f.fileUrl.includes('/Procedes/')))) {
...
driveFiles.forEach((df, idx) => {
  if (!df._matched) {
    ...
    const newId = `ffm_proc_${Date.now()}_${idx + 1}`;
    
    let category = 'ملخصات ودروس';
    if (cleanTitle.includes('تمارين') || cleanTitle.includes('سلسلة')) category = 'سلاسل وتمارين';
    else if (cleanTitle.includes('حلول') || cleanTitle.includes('الكتاب المدرسي')) category = 'حلول الكتاب المدرسي';

    const author = cleanTitle.includes('لبتر بوعلام') ? 'الأستاذ لبتر بوعلام' : 'أساتذة متميزون';
    ...
    files.push({
      ...
      subjectAliases: ['genie_procedes', 'procedes', 'gp', 'genie_des_procedes'],
...
console.log('Updated existing Génie des Procédés files:', updatedCount);
console.log('Appended new Génie des Procédés Drive files:', newCount);
