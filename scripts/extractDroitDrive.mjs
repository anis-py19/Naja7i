...
const folderUrl = 'https://drive.google.com/drive/folders/1lfNm4nOZIQsGvIXb8QqIPquBtTZCyHge?usp=sharing';
...
async function extractDroitDrive() {
  console.log('Fetching Droit (القانون) Google Drive folder...');
  const html = await fetchHtml(folderUrl);
  fs.writeFileSync('scripts/drive_droit_raw.html', html);
  ...
  console.log('Total unique Droit Drive files:', unique.length);
  fs.writeFileSync('scripts/droit_drive_files.json', JSON.stringify(unique, null, 2));
...
extractDroitDrive().catch(console.error);
