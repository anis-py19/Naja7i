...
const folderUrl = 'https://drive.google.com/drive/folders/1ETr1PjtWZPq1Br06pHPfvoAZVO4SVP76?usp=sharing';
...
async function extractComptaDrive() {
  console.log('Fetching Compta Google Drive folder...');
  const html = await fetchHtml(folderUrl);
  fs.writeFileSync('scripts/drive_compta_raw.html', html);
  ...
  console.log('Total unique Compta Drive files:', unique.length);
  fs.writeFileSync('scripts/compta_drive_files.json', JSON.stringify(unique, null, 2));
...
extractComptaDrive().catch(console.error);
