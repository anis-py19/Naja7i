...
const driveFiles = JSON.parse(fs.readFileSync('scripts/droit_drive_files.json', 'utf8'));
...
files.forEach(f => {
  if (f.subjectId === 'droit' || f.subjectId === 'law' || f.subjectId === 'qanon' || (f.fileUrl && (f.fileUrl.includes('/Droit/') || f.fileUrl.includes('/Qanon/')))) {
...
driveFiles.forEach((df, idx) => {
  if (!df._matched) {
    ...
    const newId = `ffm_droit_${Date.now()}_${idx + 1}`;
    
    let category = 'ملخصات ودروس';
    const author = cleanTitle.includes('يوسي قادة') ? 'الأستاذ يوسي قادة'
      : cleanTitle.includes('عبدالخالق') || cleanTitle.includes('عبد الخالق') ? 'الأستاذ عبد الخالق'
      : 'أساتذة متميزون';
    ...
    files.push({
      ...
      extension: 'pdf',
      subjectId: 'droit',
      subjectAliases: ['droit', 'law', 'qanon', 'kanoun'],
      subjectName: 'القانون',
      streams: ['gestion'],
      streamIds: ['gestion'],
      category,
...
console.log('Updated existing Droit files:', updatedCount);
console.log('Appended new Droit Drive files:', newCount);
