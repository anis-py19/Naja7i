...
const folderUrl = 'https://drive.google.com/drive/folders/1wwN-cohGuD4Ehr2bJbi2wnmcVXIwqrHQ?usp=sharing';
...
async function extractMecaniqueDrive() {
  console.log('Fetching Génie Mécanique (الهندسة الميكانيكية) Google Drive folder...');
  const html = await fetchHtml(folderUrl);
  fs.writeFileSync('scripts/drive_mecanique_raw.html', html);
  ...
  console.log('Total unique Génie Mécanique Drive files:', unique.length);
  fs.writeFileSync('scripts/mecanique_drive_files.json', JSON.stringify(unique, null, 2));
...
extractMecaniqueDrive().catch(console.error);
