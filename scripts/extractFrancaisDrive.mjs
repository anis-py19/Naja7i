...
const folderUrl = 'https://drive.google.com/drive/folders/1eDB7DwzMCtktZ3GAt6-Qp-NJEn7kz5_A?usp=sharing';
...
async function extractFrancaisDrive() {
  console.log('Fetching Français (اللغة الفرنسية) Google Drive folder...');
  const html = await fetchHtml(folderUrl);
  fs.writeFileSync('scripts/drive_francais_raw.html', html);
  ...
  console.log('Total unique Français Drive files:', unique.length);
  fs.writeFileSync('scripts/francais_drive_files.json', JSON.stringify(unique, null, 2));
...
extractFrancaisDrive().catch(console.error);
