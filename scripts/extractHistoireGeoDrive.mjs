...
const folderUrl = 'https://drive.google.com/drive/folders/1M7Iv-gxSZJs9fCzvFyPQXfvizukam4vs?usp=sharing';
...
async function extractHistoireGeoDrive() {
  console.log('Fetching Histoire-Géo (التاريخ والجغرافيا) Google Drive folder...');
  const html = await fetchHtml(folderUrl);
  fs.writeFileSync('scripts/drive_histgeo_raw.html', html);
  ...
  console.log('Total unique Histoire-Géo Drive files:', unique.length);
  fs.writeFileSync('scripts/histgeo_drive_files.json', JSON.stringify(unique, null, 2));
...
extractHistoireGeoDrive().catch(console.error);
