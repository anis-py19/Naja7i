...
const folderUrl = 'https://drive.google.com/drive/folders/1VgEecUjkdLgTerdeepW2CcLLB_MdsOn_?usp=sharing';
...
async function extractPhysiqueDrive() {
  console.log('Fetching Physique (العلوم الفيزيائية) Google Drive folder...');
  const html = await fetchHtml(folderUrl);
  fs.writeFileSync('scripts/drive_physique_raw.html', html);
  ...
  console.log('Total unique Physique Drive files:', unique.length);
  fs.writeFileSync('scripts/physique_drive_files.json', JSON.stringify(unique, null, 2));
...
extractPhysiqueDrive().catch(console.error);
