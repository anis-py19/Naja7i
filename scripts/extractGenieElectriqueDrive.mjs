...
const folderUrl = 'https://drive.google.com/drive/folders/1OZqVXEmz2VClQllUVkPeeJXADZBAfuDU?usp=sharing';
...
async function extractGenieElectriqueDrive() {
  console.log('Fetching Génie Électrique (الهندسة الكهربائية) Google Drive folder...');
  const html = await fetchHtml(folderUrl);
  fs.writeFileSync('scripts/drive_electrique_raw.html', html);
  ...
  console.log('Total unique Génie Électrique Drive files:', unique.length);
  fs.writeFileSync('scripts/electrique_drive_files.json', JSON.stringify(unique, null, 2));
...
extractGenieElectriqueDrive().catch(console.error);
