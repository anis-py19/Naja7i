...
const folderUrl = 'https://drive.google.com/drive/folders/19uhYCOWMaIVTtPrsxVX35seuldlGu2NI?usp=sharing';
...
async function extractScienceDrive() {
  console.log('Fetching Science (علوم الطبيعة والحياة) Google Drive folder...');
  const html = await fetchHtml(folderUrl);
  fs.writeFileSync('scripts/drive_science_raw.html', html);
  ...
  console.log('Total unique Science Drive files:', unique.length);
  fs.writeFileSync('scripts/science_drive_files.json', JSON.stringify(unique, null, 2));
...
extractScienceDrive().catch(console.error);
