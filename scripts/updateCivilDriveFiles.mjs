...
const driveFiles = JSON.parse(fs.readFileSync('scripts/civil_drive_files.json', 'utf8'));
...
files.forEach(f => {
  if (f.subjectId === 'genie_civil' || (f.fileUrl && f.fileUrl.includes('/GenieCivil/'))) {
...
driveFiles.forEach((df, idx) => {
  if (!df._matched) {
    ...
    const newId = `ffm_civil_${Date.now()}_${idx + 1}`;
    
    let category = 'ملخصات ودروس';
    if (cleanTitle.includes('سلسلة') || cleanTitle.includes('تمارين') || cleanTitle.includes('تطبيقات')) category = 'سلاسل وتمارين';
    else if (cleanTitle.includes('حلول') || cleanTitle.includes('الكتاب المدرسي')) category = 'حلول الكتاب المدرسي';

    const author = cleanTitle.includes('سعدي') ? 'الأستاذ سعدي إسماعيل'
      : cleanTitle.includes('مخلوفي') ? 'الأستاذ مخلوفي كمال'
      : 'أساتذة متميزون';
    ...
    files.push({
      ...
      subjectAliases: ['genie_civil', 'civil', 'gc'],
...
console.log('Updated existing Génie Civil files:', updatedCount);
console.log('Appended new Génie Civil Drive files:', newCount);
