...
const folderUrl = 'https://drive.google.com/drive/folders/1KeuUVE4lLW2m0uf0z9Ig2FDQo1y9bW7x?usp=sharing';
...
async function extractNourEddineDrive() {
  console.log('Fetching Math - Prof Noureddine Google Drive folder...');
  const html = await fetchHtml(folderUrl);
  fs.writeFileSync('scripts/drive_noureddine_raw.html', html);
  ...
  console.log('Total unique Noureddine Math Drive files:', unique.length);
  fs.writeFileSync('scripts/noureddine_drive_files.json', JSON.stringify(unique, null, 2));
...
extractNourEddineDrive().catch(console.error);
