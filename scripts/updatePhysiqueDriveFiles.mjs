...
const driveFiles = JSON.parse(fs.readFileSync('scripts/physique_drive_files.json', 'utf8'));
...
files.forEach(f => {
  if (f.subjectId === 'physique' || f.subjectId === 'physics' || f.subjectId === 'chimie' || (f.fileUrl && (f.fileUrl.includes('/Physique/') || f.fileUrl.includes('/Physics/')))) {
...
driveFiles.forEach((df, idx) => {
  if (!df._matched) {
    ...
    const newId = `ffm_phy_${Date.now()}_${idx + 1}`;
    
    let category = 'ملخصات ودروس';
    if (cleanTitle.includes('بروتكول') || cleanTitle.includes('بروتوكول') || cleanTitle.includes('التحليل البعدي')) category = 'بروتوكولات تجريبية ومنهجية';
    else if (cleanTitle.includes('تمارين') || cleanTitle.includes('سلسلة') || cleanTitle.includes('باقة')) category = 'سلاسل وتمارين';
    else if (cleanTitle.includes('الكتاب المدرسي') || cleanTitle.includes('حلول')) category = 'حلول الكتاب المدرسي';
    else if (cleanTitle.includes('قوانين') || cleanTitle.includes('تعاريف')) category = 'قوانين ومفاهيم أساسية';
    else if (cleanTitle.includes('Almondjid') || cleanTitle.includes('المنجد')) category = 'سلسلة المنجد';

    const author = cleanTitle.includes('قزوري') ? 'الأستاذ قزوري'
      : cleanTitle.includes('عايب') ? 'الأستاذ عايب كمال'
      : cleanTitle.includes('مبخوت') ? 'الأستاذ مبخوت بقة'
      : cleanTitle.includes('بلعمري') ? 'الأستاذ بلعمري'
      : 'أساتذة متميزون';
    ...
    files.push({
      ...
      extension: 'pdf',
      subjectId: 'physique',
      subjectAliases: ['physique', 'physics', 'fizya', 'chimie'],
      subjectName: 'العلوم الفيزيائية',
      streams: scientificStreams,
      streamIds: scientificStreams,
      category,
      author,
...
console.log('Updated existing Physique files:', updatedCount);
console.log('Appended new Physique Drive files:', newCount);
