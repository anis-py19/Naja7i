                ...
                <div className="prose prose-sm max-w-none text-[#0F172A] leading-relaxed space-y-4">
                  {summaryResult.split('\n').map((paragraph, idx) => {
                    ...
                    if (trimmed.startsWith('# ')) {
                      return <h1 key={idx} className="text-lg sm:text-xl font-black text-[#0F172A] border-b pb-2 mt-4 text-[#E11D48]">{trimmed.replace('# ', '')}</h1>;
                    }
                    if (trimmed.startsWith('## ')) {
                      return <h2 key={idx} className="text-base font-black text-[#0F172A] mt-4 mb-2 flex items-center gap-1.5 text-[#0F172A]">{trimmed.replace('## ', '')}</h2>;
                    }
                    if (trimmed.startsWith('### ')) {
                      return <h3 key={idx} className="text-sm font-bold text-[#0F172A] mt-3 mb-1">{trimmed.replace('### ', '')}</h3>;
                    }
                    if (trimmed.startsWith('- ') || trimmed.startsWith('• ') || trimmed.startsWith('* ')) {
                      return (
                        <div key={idx} className="flex items-start gap-2 text-xs sm:text-sm text-[#334155] my-1 pr-2">
                    ...
                    if (trimmed.startsWith('> ')) {
                      return (
                        <div key={idx} className="p-3 rounded-xl bg-rose-50/70 border border-rose-200 text-rose-950 text-xs leading-relaxed my-2 font-medium">
                          {trimmed.replace('> ', '')}
                        </div>
                      );
                    }
                    ...
                    return (
                      <p key={idx} className="text-xs sm:text-sm text-[#475569] leading-relaxed">
                        {trimmed}
                      </p>
                    );
                  })}
