...
| 🎨 **`ui_frontend_agent`**        | **هندسة الواجهات وتجربة المستخدم والمستعرض**<br>تصميم React 19، Tailwind v4، ومستعرض PDF (Canvas/Drive).<br>_(معيار الشارات: جميع الـ Badges بسيطة، هادئة، وبستايل Minimalist ناعم)_. | `src/components/Navbar.jsx`<br>`src/components/PdfReaderModal.jsx`<br>`src/App.jsx`<br>`src/index.css`                           |
...
## ⏱️ 4. Scheduled Task Agents Ecosystem (جدول المهام المجدولة الدائمة)

تم تفعيل وجدولة مهام دورية دائمة (Recurring Background Daemons) لكل وكيل من وكلاء المنصة لمراقبة وتحديث وصيانة النظام بشكل ذاتي:

| الوكيل (Agent) | توقيت الجدولة (Cron) | المهمة المجدولة الدائمة (Scheduled Task Action) | حالة التشغيل |
| :--- | :--- | :--- | :--- |
| 📚 **`lessons_curriculum_agent`** | `0 6 * * *` (يومياً 06:00) | **فحص وتدقيق سلامة ملفات المنهاج (330 ملف Drive):** التأكد من فاعلية الروابط والتصنيفات لجميع الشعب الست. | 🟢 Daemon Active (`task-2802`) |
| 🏛️ **`bac_archive_agent`** | `0 7 * * *` (يومياً 07:00) | **تدقيق مواضيع وسلم تنقيط البكالوريا (2008-2026):** مطابقة الروابط ونماذج الإجابة الوزارية الرسمية. | 🟢 Daemon Active (`task-2804`) |
| ⏱️ **`quiz_engine_agent`** | `0 8 * * *` (يومياً 08:00) | **مراجعة وتحديث بنك الأسئلة QCM:** فحص دقة الخيارات الأربعة والشروحات المنهجية المعتمدة. | 🟢 Daemon Active (`task-2806`) |
| 🧮 **`smart_tools_agent`** | `0 9 * * *` (يومياً 09:00) | **تدقيق المعاملات والمخططات وحاسبة المعدل:** مطابقة المعاملات الرسمية وجاهزية قوالب الطباعة A4. | 🟢 Daemon Active (`task-2808`) |
| 🎥 **`youtube_media_agent`** | `0 10 * * *` (يومياً 10:00) | **مراجعة قنوات وقوائم تشغيل أساتذة اليوتيوب:** التحقق من صلاحية الروابط وتحديث أساليب الشرح. | 🟢 Daemon Active (`task-2810`) |
| 🎨 **`ui_frontend_agent`** | `0 11 * * *` (يومياً 11:00) | **فحص جودة الواجهات وتكامل البناء (UI/UX Health):** اختبار `npm run build` وتطبيق معيار الشارات البسيطة ومستعرض PDF. | 🟢 Daemon Active (`task-2812`) |
